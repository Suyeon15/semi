...
//========= 게시판 기본 메서드 ======================================================================	
	...
	public Board2DTO detail(int board_seq2) throws Exception {
		String sql ="select * from board2 where board_seq2=?";
		...
		try(Connection con = this.getConnection();
			...
			try(ResultSet rs =pstat.executeQuery();){
				if(rs.next()) {
					dto.setBoard_seq(rs.getInt("board_seq2"));
					dto.setId(rs.getNString("id2"));
					dto.setTitle(this.ReXSSFilter(rs.getString("title2")));
					dto.setContent(rs.getString("content2"));
					dto.setWrite_date(rs.getDate("write_date2"));
					dto.setView_count(rs.getInt("view_count2"));
					dto.setNotice(rs.getNString("notice2"));
				}
				return dto;
			}
		}	
	...
	public List<Board2DTO> getPageList(int startNum, int endNum) throws Exception {
		...
		try (Connection con = this.getConnection(); PreparedStatement pstat = con.prepareStatement(sql);) {
			...
			try (ResultSet rs = pstat.executeQuery();) {
				...
				while (rs.next()) {
					int board_seq = rs.getInt("board_seq2");
					String id = rs.getNString("id2");
					String title = this.ReXSSFilter(rs.getString("title2"));
					String content = rs.getString("content2");
					Date write_date = rs.getDate("write_date2");
					int view_count = rs.getInt("view_count2");
					String notice = rs.getNString("notice2");
					list.add(new Board2DTO(board_seq,id,title,content,write_date,view_count,notice));
				}
				return list;
			}
